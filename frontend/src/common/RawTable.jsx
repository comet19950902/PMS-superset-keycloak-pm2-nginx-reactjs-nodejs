...
export default function RawTable({search,loading,data,columns}) {
  const [result,setResult]=useState([]);
    ...
    const exchange = async () => {
      ...
      await axios(config).then(function (response) {
       ...
       if(search!=undefined){
        setResult(response.data.filter(i=>search.some(y=>y.exchange_name==i.exchange_name)))
       }else{
...
const Record= loading ? "" :  !(result1.length>0) && 'no record found'
  return (
      <div className="container_exchange">
       <div className="container_row_fixed">
             ...
             {result.length>0 ? <></> :
           <tr><th style={{color:'white',fontSize:'14px',fontWeight:'100',borderBottom:'none',left:'45%'}}>{Record}</th> </tr> }
