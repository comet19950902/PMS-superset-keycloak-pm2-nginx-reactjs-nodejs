...
const get_history_list = async(address_id,res)=>{
            ...
            try {
              // let assetidinuse = 'eth'
                ...
                for (const element of chainresult.data.history_list) {
                  ...
                  // let amount_token_id = Object.entries(JSON.parse(JSON.stringify(element.tx)))
                  // console.log("apount token id",amount_token_id)
                  // let usdt_type = Object.values(JSON.parse(JSON.stringify(symbol_of_data.token_dict)).filter (i =>i.id ==amount_token_id.to_addr ))[0]
                  // console.log("usdt type ",usdt_type)
                  // console.log("ammount of the symbol ",Object.values(JSON.parse(JSON.stringify(symbol_of_data.token_dict))))
              ...
              let comments =  comment_data ? comment_data :null;
                  ...
                  try {
                    let sql = "REPLACE INTO pms_wallet_transaction_history_list (transaction_id, other_wallet_address, asset_chain, amount, transaction_time, cate_id, address_id, send_data, recieve_data, created_at,symbol,usdt_eth,eth_fee,token_type,eth_time,gas_fee,amount_returned,comments) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);"
...
module.exports = get_history_list