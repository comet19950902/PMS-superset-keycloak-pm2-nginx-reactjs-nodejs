...
const get_btc_audit_logs = async(res)=>{
    try {
        db.getConnection(async (err, connection) => {
        if (err) throw err
        const execquery = util.promisify(connection.query.bind(connection))
            try {
                const sqlSearch1 = "Select * from btc_audit_logs ORDER BY date_create DESC"
                let searchresult1 =await execquery(sqlSearch1)      
                    // console.log(searchresult1)
                    let transaction_data = searchresult1;
                    const sqlSelect2 = "Select * from btc_balance"
                    ...
                    let address_data = result2;
                        const all_audit_result = transaction_data.map(transactiondata => {
                            const addressItem = address_data.find(addresses => addresses.btc_address_id === transactiondata.btc_address_id) 
...
module.exports = get_btc_audit_logs