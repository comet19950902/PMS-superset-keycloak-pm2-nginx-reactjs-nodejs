...
const load_btc_trx = async (btc_address_id, address_type,load_number,comment, res) => {
    ...
    try {
        db.getConnection(async (err, connection) => {
            ...
            try {
                ...
                var sql_data_len = "Select * FROM btc_txc WHERE btc_address_id =?";
                resultdata1_data_len = await execquery(sql_data_len, [btc_address_id]);
                var total_len = resultdata1_data_len.length + load_number
                console.log(resultdata1_data_len.length + load_number)
    var config = {
        method: 'get',
    ...
    try {
        db.getConnection(async (err, connection) => {
            ...
            try{
                ...
                // let resultdata_for_all = await execquery(sqlInsert_for_all, [btcData.txs[i].hash,btcData.txs[i].time,btcData.txs[i].result,comment,btc_address_id,to_address,portfolio_id, wallet_id,address_type,date,cate_id,send_data,recieve_data,btcData.txs[i].fee,tron_fee,btcData_usd_fee_for_tab,btcData_usd_result,symbol,tokenDecimal,tron_usd_fee,method,usdt_token_value,usdc_value,tokenAbbr])
                //     console.log("all tra",resultdata_for_all);
                //     console.log(resultdata_for_all);
                }
            }
