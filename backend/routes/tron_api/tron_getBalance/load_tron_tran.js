...
var resultdata1_data_len

const load_tron_transaction = async (address_id, address_type,start,comment, res) => {
    console.log("tron",address_id);
    try {
        db.getConnection(async (err, connection) => {
            ...
            try {
                ...
                var sql_data_len = "Select * FROM tron_transactions WHERE address_id =?";
                resultdata1_data_len = await execquery(sql_data_len, [address_id]);
                var total_len = resultdata1_data_len.length + start
                console.log(resultdata1_data_len.length + start)
                // var config = {
                //     url: `https://apilist.tronscanapi.com/api/transaction?sort=-timestamp&count=true&limit=20` +`&start=` + resultdata1_data_len.length + start + `&address=` + address_id,
                //     headers: {}
    ...
    var config = {
        url: `https://apilist.tronscanapi.com/api/transaction?sort=-timestamp&count=true&limit=50` +`&start=` + total_len + `&address=` + address_id,
        headers: {}
...
module.exports = load_tron_transaction; 