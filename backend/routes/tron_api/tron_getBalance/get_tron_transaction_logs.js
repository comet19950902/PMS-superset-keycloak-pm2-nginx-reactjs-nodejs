...
const get_tron_audit_history_list = async(res)=>{
    try {
        db.getConnection(async (err, connection) => {
        if (err) throw err
        const execquery = util.promisify(connection.query.bind(connection))
            try {
                const sqlSearch1 = "Select * from tron_transaction_audit_logs ORDER BY timestamp DESC"
                let searchresult1 =await execquery(sqlSearch1)      
                    // console.log(searchresult1)
                    let transaction_data = searchresult1;
                    const sqlSelect2 = "Select * from tron_balance"
                    ...
                    let address_data = result2;
                        const all_audit_result = transaction_data.map(transactiondata => {
                            const addressItem = address_data.find(addresses => addresses.address_id === transactiondata.address_id) 
...
module.exports = get_tron_audit_history_list