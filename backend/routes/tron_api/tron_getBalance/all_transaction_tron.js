...
const all_trondata_transaction = async (address_id, address_type, res) => {
    let comment_data
    let tron_last_time;
    console.log(address_id);
                ...
                // console.log("ressss", result_of_prev)
                var tronData = result_of_prev.data;
                // console.log("trondata", tronData)
                let transaction_of_data = tronData?.total
                // console.log("trannnnnn", transaction_of_data)
                let transactionData
                ...
                try {
                    db.getConnection(async (err, connection) => {
                        ...
                        var rcv_amount = 0;
                        var send_amount = 0;
                for (let i = 0; i <= transaction_of_data; i+= 15) {
                    s = s + i
                    var config = {
                        method: 'get',
                        url: `https://apilist.tronscanapi.com/api/transaction?sort=-timestamp&count=true&limit=50&start=` + s + `&address=` + address_id,
                        headers: {}
                    };
                    console.log("start")
                    // var config_for_all_tron = {
                    //     method: 'get',
                    //     url: `https://apilist.tronscanapi.com/api/transaction?sort=-timestamp&count=true&limit=50&start=` + i + `&address=` + address_id,
                    //     headers: {}
                    // };
                    ...
                    var config_for_all_tron = {
                        method: 'get',
                        url: `https://apilist.tronscanapi.com/api/transaction?sort=-timestamp&count=true&limit=50&start=` + result.data.length + `&address=` + address_id,
                        headers: {}
                    };
                    let resultdata_all_tron = await axios(config_for_all_tron);

                    transactionData = result.data; 
                    tron_last_time = (transactionData.data).slice(0)?.[0]?.timestamp;                  
                    let transactionData_data = resultdata_all_tron.data
                    // console.log("all tran", transactionData_data)
                    // for(let i =50;i<=10000;i++){
                    //     result = await axios(config_for_all_tron) 
                    //     transactionData = result.data
                    // }
                        ...
                        //     const execquery = util.promisify(connection.query.bind(connection))
                            try {
                                ...
                                for (let i = 0; i < transactionData.data.length; i++) {
                                    ...
                                    if (transactionData.data[i].owner_address != address_id){
                                        rcv_amount += parseInt(transactionData.data[i].amount / 1000000) * usd_tron_fee.data[0]?.[2]
                                    }
                                    else {
                                        send_amount += usdc_value + usdt_value
                                    }
                                    ///////////////////////////////////////////
                    ...
                    // const sqlSelect_tron = "Update tron_balance SET tron_last_time=?,rcv_amount=?,send_amount=?  WHERE address_id =?"
                    // let result_tron =await execquery(sqlSelect_tron,[tron_last_time,rcv_amount,send_amount,address_id])       
                    // console.log("connection address",result_tron); 
                        ...
                        console.log("send and rcv amount",rcv_amount, send_amount)
                ...
                console.log("tron last time",tron_last_time);
                res.send(transactionData);
...
module.exports = all_trondata_transaction; 